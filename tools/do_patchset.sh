...
function usage() {
	...
	echo "-c|--codedir <value>: The code dir for patchset"
	echo "-o|--outdir <value>: Output directory.  Default is /tmp/patches"
	echo "-s|--startcommit <value>: The start commit id in src branch"
	echo "-e|--endcommit  <value>: The end commit id. Default is HEAD"
}
...
git log --oneline $START_COMMIT..$END_COMMIT $CODE_DIR > /tmp/patch
